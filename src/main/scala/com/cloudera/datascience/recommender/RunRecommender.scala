...
object RunRecommender {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length != 1) {
      println("You must introduce a user ID as argument")
      return
    }
...
//    val spark = SparkSession.builder().getOrCreate()
    ...
    val sparkConf = new SparkConf().setAppName("Recommender").setMaster("local[2]").set("spark.executor.memory","1g");
...
//    spark.sparkContext.setCheckpointDir("/home/pablo/Escritorio/aas/ch03-recommender/")
    ...
    val runRecommender = new RunRecommender(spark, userID)
...
class RunRecommender(private val spark: SparkSession, private val usr: Int) {
